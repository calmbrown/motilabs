...
class _NoteItemWidgetState extends State<NoteItemWidget> {
  ...
  Widget build(BuildContext context) {
    return ListView.separated(
      ...
      itemCount: widget.mainList.length, // make sure mainList is accessible here
      itemBuilder: (BuildContext context, int index) {
        return ListTile(
          title: Container(
            ...
            child: Text(widget.mainList[index]['title'], textAlign: TextAlign.start,),
          ),
          trailing: widget.isEditMode // 편집 모드일 경우에만 삭제 버튼 표시
              ...
              : null,
          onTap: () {
            print(widget.mainList[index]);
            final result = Navigator.push(
                context,
                MaterialPageRoute(
                    builder: (context) => MemoScreen(
                          // noteTitle: widget.mainList[index]['title'],
                          mainList: widget.mainList[index]['id'],
                        )));
            if (result == 'updated') {
              setState(() {});
